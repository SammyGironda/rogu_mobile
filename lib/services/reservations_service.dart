...
class ReservationSlot {
  ...
  final bool ocupado;
  ReservationSlot({required this.horaInicio, required this.horaFin, required this.ocupado});
}

class ReservationsService {
  final String baseUrl;
  ReservationsService({String? baseUrl}) : baseUrl = baseUrl ?? AppConfig.apiBaseUrl;

  Future<List<Map<String, dynamic>>> getReservationsForField(int idCancha, DateTime date) async {
  ...
  Future<List<Map<String, dynamic>>> getReservationsForUser(int idUsuario) async {
  ...
  List<ReservationSlot> buildSlots(List<Map<String, dynamic>> existing, {int inicioHora = 6, int finHora = 24}) {
    ...
    for (int h = inicioHora; h < finHora; h++) {
      ...
      final ocupado = existing.any((r) => overlap(r['horaInicio'] as String, r['horaFin'] as String, start, end));
      slots.add(ReservationSlot(horaInicio: start, horaFin: end, ocupado: ocupado));
    }
    return slots;
  }
  ...
  Future<Map<String, dynamic>> createReservation({
    ...
    if (res.statusCode != 201 && res.statusCode != 200) {
      throw Exception('Error al crear reserva (${res.statusCode}): ${res.body}');
    }
    final data = jsonDecode(res.body);
