...
class ProfileService {
  ...
  Future<Map<String, dynamic>> changePassword({
    ...
    final url = Uri.parse('${AppConfig.apiBaseUrl}/usuarios/$userId/cambiar-contrasena');
    try {
  ...
  Future<Map<String, dynamic>> changePasswordWithCurrent({
    ...
    try {
      final loginRes = await http.post(loginUrl, headers: {
        'Content-Type': 'application/json',
      }, body: jsonEncode({
        'correo': userEmail,
      ...
      return {'success': false, 'message': 'Error validando contraseña actual: $e'};
    }

    // Si valida, cambiar contraseña
    return await changePassword(token: token, userId: userId, newPassword: newPassword);
  }
}
