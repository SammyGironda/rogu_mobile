...
class GallerySection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    List<Field> itemsBySport = baseFields.where((i) {
      ...
      switch (filter) {
        case 'todo':
          return true;
        case 'futbol':
          return sportLower.contains('fútbol') || sportLower.contains('futbol');
        case 'voley':
          return sportLower.contains('vóley') || sportLower.contains('voley');
        case 'basket':
          return sportLower.contains('básquet') || sportLower.contains('basket') || sportLower.contains('basquet');
        case 'multidisciplinas':
          // Consider multi-sport courts like futsal, pádel, etc. under multidisciplinas
          return !(sportLower.contains('fútbol') || sportLower.contains('futbol') || sportLower.contains('básquet') || sportLower.contains('basket') || sportLower.contains('basquet'));
        default:
          return true;
      }
    }).toList();
    ...
    return Column(
      ...
      children: [
        Text('Galería de canchas', style: Theme.of(context).textTheme.titleLarge),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text(item.nombre, style: Theme.of(context).textTheme.titleMedium),
                    ),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 12),
                      child: Text(item.deporte ?? 'Multidisciplinas', style: Theme.of(context).textTheme.bodySmall),
                    ),
  ...
  void _openFieldDetail(BuildContext context, Field field) {
    final venue = Venue(id: 0, nombre: 'Sede', deportesDisponibles: const [], canchas: const []);
    Navigator.push(context, MaterialPageRoute(builder: (_) => FieldDetailScreen(field: field, venue: venue)));
  }
}
